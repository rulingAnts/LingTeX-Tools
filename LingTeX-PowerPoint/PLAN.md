...
- **The Mac's numbers follow PowerPoint's AppleScript save formats** (`PowerPoint.sdef`, enumeration `EPPSaveAsFileType`, the low byte of each code): 1 presentation, 5 template, 6 RTF, 7 show, 8 add-in (the old binary `.ppa`), 10 default, 11 HTML, 12 movie, 13 package, 14 PDF, 15 Open XML presentation (`.pptx`), 16 Open XML macro-enabled presentation (`.pptm`), 17 and 18 Open XML shows, 19 and 20 Open XML templates (`.potx`, `.potm`), 21 theme. Every number that wrote a file or gave a specific message matches that list. There is no Open XML add-in (`.ppam`) in it.
- **So 14 to 21 most likely save in the background.** They reported no error and left no file, probably because PowerPoint crashed (below) before they finished. 16 is very likely a working `.pptm` save from VBA. Not confirmed, and not needed.
