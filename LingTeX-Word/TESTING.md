...
| `LingTeXMergeColumns` with the cursor in `kada` → merged back to `kada=te` / `carry.CMP=SEQ` | ☐ | ☐ |
| Select across three cells and merge → exactly those three become one | ☐ | ☐ |
| Split a column whose gloss has no matching break (e.g. form `kata-bi`, gloss `gone`) → a message naming the tier; the gloss cell stays whole and the new column is empty for it; **nothing is guessed** | ☐ | ☐ |
...
| Make two tiers claim different break characters (`-bi` over `=DIST`) → reported as a conflict and **not** silently changed | ☐ | ☐ |
| `kada=te` over `carry.CMP=SEQ` → **no** rule-2 warning (`.` and `:` are not morpheme breaks) | ☐ | ☐ |
...
| Type three lines: `(1) di=de deda=di bujo`, `pig=ERG attack.CMP=REL speak`, `'A pig attacked her.'`; select them, **Text to Interlinear**, accept the guess of 1 → a numbered example with three columns and the translation; by morpheme (Settings) the same text gives five columns | ☐ | ☐ |
| Type a space into a gloss cell (`attack CMP=REL`), **Re-wrap This** → it reads `attack.CMP=REL` (or `_` if the document says so); the translation keeps its spaces | ☐ | ☐ |
