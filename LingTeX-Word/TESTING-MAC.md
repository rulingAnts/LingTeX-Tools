...
- **Shortcuts.** Run `LingTeXInstallShortcuts` (from the Macros dialog, one
  last time). It binds every command to ⌘⌥⇧ + a letter (Ctrl+Alt+Shift on
  Windows), so they work in every document: I insert, R re-wrap this, A
...
4. Make two tiers claim *different* break characters — `-bi` over `=DIST` → a
   conflict, reported and **not** silently resolved.
5. `kada=te` over `carry.CMP=SEQ` → no rule-2 warning; `.` and `:` are not
   morpheme breaks. Change it to `carry-CMP=SEQ` → a rule-2 warning.
