...
Private Sub TestSpaceFix()
    ...
    End If
    ex = ReadExampleFromTable(tbl)
    Eq "re-wrap replaces the space with the document's full stop", ex.Cells(1, 1), "follow.CMP=REL"
    Ok "the vernacular cell above it is untouched", (ex.Cells(0, 1) = "levo=zi")
    ...
    If Not tbl Is Nothing Then
        ex = ReadExampleFromTable(tbl)
        Eq "re-wrap uses an underscore when the document says so", ex.Cells(0, 2), "zu_vo"
    Else
...
Private Sub TestTextToInterlinear()
    ...
    If doc.Tables.Count = 1 Then
        ...
        Eq "  starting with the first word", back.Cells(0, 0), "Uwzob"
    End If
...
Private Sub CheckTransform(doc As Document)
    ...
    Eq "transform: follow.CMP on a gloss row keeps a full-size first capital", _
    ...
    Eq "transform: without the initial capital, uniform small caps", _
        TransformedCellText("follow.CMP=REL", ROLE_GLOSS, doc), "follow.cmp=rel"
    SetSettingGramGlossInitialCap doc, True
...
Private Sub CheckSmallCapsRuns(tbl As Table)
    ...
    For r = 1 To tbl.Rows.Count
        For c = 1 To tbl.Rows(r).Cells.Count
            ...
            If LCase$(Left$(txt, 10)) = "follow.cmp" Then
                found = True
                Eq "follow.CMP=REL is drawn in small-caps case", txt, "follow.Cmp=Rel"
