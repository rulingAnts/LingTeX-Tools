...
**Input:**
```
Word the fox followed voxi
WordGloss DEF fox follow.PST P.N.
```
...
- [ ] Frame 1 vernacular row: *zomu-xa-vi* (morphemes collapsed, dividers kept) — **italic**
- [ ] Frame 1 gloss row: `go-dist-nfut` — `dist` and `nfut` in **small caps**, `-` delimiters in normal weight
- [ ] Frame 2 vernacular: *-xa* — italic (leading dash retained)
...
**Input:**
```
Morphemes vu =ve levo =zi
LexGloss fox =ERG follow.CMP =REL
```

**Expected:**
- [ ] Two frames (two word groups separated by space)
- [ ] Frame 1 vernacular: *vu=ve* — italic
- [ ] Frame 1 gloss: `fox=erg` — `fox` normal, `=` normal, `erg` **small caps**
- [ ] Frame 2 vernacular: *levo=zi* — italic
- [ ] Frame 2 gloss: `follow.cmp=rel` — `follow` normal, `.` normal, `cmp` small caps, `=` normal, `rel` small caps
...
- [ ] Row 1 (Word): *zomu-xa-vi* — italic
- [ ] Row 2 (Morphemes): *zomu-xa-vi* — italic (also vernacular tier index > 0? No — Row 0 = first line, so this should also be italic since t=0 is `Word`, t=1 is `Morphemes`)
...
```
Word vu levo zo
Morphemes vu levo =zi zo
LexGloss fox follow.CMP =REL FOC
WordGloss the.fox followed-REL FOC
```
...
- [ ] Free translation text: *'The fox followed Voxi.'* — wrapped in typographic single quotes (' ')
...
| `A.` | NOT small caps |
| `DIST` | small caps |
| `follow.CMP` | `follow` normal, `.` normal, `CMP` small caps |
...
WordGloss FOC 3SG 3sg bark P.N. N. A. DIST follow.CMP =ERG 1s.POSS
```
...
- [ ] One frame: vernacular *vu=ve*, gloss `fox=erg`
...
```
Morphemes vu =ve levo =zi zo
LexGloss fox =ERG follow.CMP =REL FOC
Free The fox followed [him], you see.
```
