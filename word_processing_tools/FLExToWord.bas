...
'   • Morphemes are COLLAPSED into words  (zomu░=░xo  →  zomu=xo)
'   • Gloss dividers are assembled inline  (stack.CMP + = + SEQ  →  stack.CMP=SEQ)
            ...
            Select Case lineTypes(t)
                ...
                Case "Morphemes", "LexEntries"
                    ...
                    If w <= UBound(wa) Then
                        ...
                        tierValues(t) = Join(morphParts, "")  ' e.g. zomu=xo
                    Else
                        missingText = True
                    End If
                ...
                Case "LexGloss"
                    ...
                    If UBound(morphWordArr) >= w And _
                       InStr(morphWordArr(w), MORPH_SEP) > 0 Then
                        ...
                        ' Assemble: "stack.CMP" + "=" + "SEQ" → "stack.CMP=SEQ"
...
Private Function BuildGlossRunXML(text As String, fontName As String) As String
    ...
    '
    ' Example:  "stack.CMP=SEQ "
    '   → segments: "stack", ".", "CMP", "=", "SEQ", " "
    '   → output:   stack  .  CMP  =  SEQ  (CMP and SEQ in small caps)
    '
    Dim segs() As String
