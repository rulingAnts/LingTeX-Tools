...
| **FLEx: whole-word export loses morpheme-boundary markers in glosses** — when the copied data has a `Word` tier (whole words) but no `Morphemes`/`Lex. Entries` tier, the morpheme glosses are reassembled per word with a neutral `-` between segments, since the original boundary marker (`-` vs `=`) is not recoverable from the word forms alone. **Workaround:** include the `Morphemes` (or `Lex. Entries`) tier in your FLEx interlinear view so segment boundaries are exported exactly. | FLEx interlinear (all platforms) |
